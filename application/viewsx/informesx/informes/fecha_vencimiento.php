...
$objPHPExcel->getProperties()
    ...
    ->setTitle('Contratos por fecha de vencimiento')
    ->setSubject("Contratos filtrados por la fecha de vencimiento")
    ->setDescription("Listado de contratos existentes a la fecha seg&uacute;n el rango de fechas seleccionado")
    ->setKeywords("fecha vencimiento final contratos excel hatovial")
    ->setCategory("Informe");
...
foreach ($contratos as $contrato):
    ...
    if( $contrato->Acta_Inicio == 1){
        //Esta sera la fecha de inicio del contrato
        $fecha_inicial = $contrato->Fecha_Acta_Inicio;
    }else{
    ...
    $fecha_vencimiento = $this->contrato_model->calcular_vencimiento($fecha_inicial, $contrato->Plazo);
   ...
   $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('F'.$fila, $fecha_vencimiento)
 ...
 */
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('A1', 'Contratos con fecha de vencimiento desde '.$fecha1.' hasta '.$fecha2)
...
if($fila == 3){
    ...
    header('Content-Disposition: attachment; filename="Contratos con vencimiento desde '.$fecha1.' hasta '.$fecha2.'.xlsx"');
