<?php 
//Se recibe el estado
$id_estado = $this->input->post('estados_contratos');

//Se carga el modelo que va a traer todos los datos
$contratos = $this->informes_model->contratos_por_estado($id_estado);
...
$objPHPExcel->getProperties()
    ...
    ->setKeywords("estado contratos excel hatovial")
    ->setCategory("Informe");
...
foreach($contratos as $contrato):
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        $objPHPExcel->getActiveSheet()->getStyle('F'.$fila)->applyFromArray($estilos_textos)->getNumberFormat();
        $objPHPExcel->getActiveSheet()->getStyle('G'.$fila)->applyFromArray($estilos_textos)->getNumberFormat();
 ...
 */
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('A1', 'Contratos '.$estado)
...
$objPHPExcel->getActiveSheet()->setTitle('Contratos '.$estado);
...
if($fila == 3){
    ...
    $this->data['mensaje_alerta'] = 'No se ha generado ning&uacute;n informe porque<br/>no hay contratos con el estado seleccionado';
    ...
    header('Content-Disposition: attachment; filename="Contratos '.$estado.'.xlsx"');
