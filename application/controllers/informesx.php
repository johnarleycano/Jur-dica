...
Class Informes extends CI_Controller{
    ...
    */
    function __construct() {
        ...
        $this->load->model(array('informes_model', 'contrato_model', 'email_model', 'pago_model', 'tercero_model', 'demanda_model', 'tipoproceso_model','tercero_model'));
    ...
    function demandas_excel(){
        //Se recibe el usuario que viene por post
        $id_demanda = $this->input->post('id_demanda');

        //Nombre genérico
        $this->data['nombre'] = "TODAS LAS DEMANDAS";
            
        //Si viene id de la demanda
        if ($id_demanda != "") {
            //Consulta de datos de la demanda
            $demanda = $this->informes_model->obtener_nombre_demanda($id_demanda);

            //Radicado de la demanda
            $this->data['nombre'] = $demandas->radicado;
        };

        //Consulta los contratos asociados a ese contratantes
        $this->data['demandas'] = $this->informes_model->listar_demanda("contratante", $id_demanda);

        // //Se carga la vista que contiene el informe
        $this->load->view('informes/informes/demandas_excel', $this->data);
    }//Fin contratantes_excel()
...
class PDF extends FPDF {
    ...
    function Footer(){
        ...
        $this->Cell(0,10, utf8_decode('Sistema de Gestión de Contratos - Hatovial S.A.S. Página ').$this->PageNo().' de {nb}',0,0,'R');
    }//Fin footer()
}//Fin PDF
