...
// $contratos = $this->contrato_model->listar_contratos($id_contrato); // consulta original
$contratos = $this->contrato_model->listar_contratos_acta_word($id_contrato);

//se trae las adiciones del contrato
$adiciones = $this->contrato_model->listar_adiciones($id_contrato);

//Se traen los pagos existentes de ese contrato
$pagos = $this->pago_model->listar_pagos($id_contrato);

// Se trae la suma de los retenidos

$retenidos = $this->pago_model->listar_suma_retenido($id_contrato);
...
$properties->setCreator('John Arley Cano Salinas - Devimed S.A.'); 
$properties->setCompany('John Arley Cano Salinas - Devimed S.A.');
...
$PHPWord->addFontStyle('parrafo3', array( 'name'=>'Arial', 'size'=> 12, 'color'=>'000000', 'bold'=> false));
$PHPWord->addFontStyle('parrafo4', array( 'name'=>'Arial', 'size'=> 12, 'color'=>'000000', 'bold'=> true));
...
$table->addCell(10000, $styleCell)->addText(utf8_decode("ACTA DE LIQUIDACIÓN"), 'titulo2', $alineacion_centrada);
...
foreach ($contratos as $contrato):
	// calculos para traer los otroSI
	...
	$seccion1->addText('VALORES DEL CONTRATO', 'parrafo1', $alineacion_izquierda);
	$seccion1->addTextBreak();

	// Valor Inicial	
	$seccion1->addText(utf8_decode('Valor Inicial:  $'.number_format($contrato->Valor_Inicial, 0, '', '.')),'parrafo3', $alineacion_justificada);
    ...
    foreach ($adiciones as $adicion):
        ...
        if($num != 0){
			...
			$seccion1->addText(utf8_decode('Otro SI No.'  .$num. ':   $'.number_format($adicion->Valor, 0, '', '.')), 'parrafo3', $alineacion_justificada);
        }
        $num++;
    endforeach;	

    $seccion1->addText(utf8_decode('Valor Total Contrato: $ '.number_format($contrato->Valor_Inicial + $contrato->Valor_Adiciones,2,',','.')), 'parrafo3', $alineacion_justificada);
	$seccion1->addTextBreak();

	// Valor total

	$seccion1->addText('VALOR TOTAL EJECUTADO POR EL CONTRATISTA', 'parrafo1', $alineacion_izquierda);
	$seccion1->addText(utf8_decode('$ '.number_format($contrato->Pagado,2,',','.').' ('.$numero->traducir($contrato->Pagado).')'), 'parrafo2', $alineacion_justificada);
	$seccion1->addTextBreak();

	$seccion1->addText('VALOR FALTANTE POR EJECUTAR DEL CONTRATISTA', 'parrafo1', $alineacion_izquierda);
	$seccion1->addText(utf8_decode('$ '.number_format(($contrato->Valor_Inicial + $contrato->Valor_Adiciones) - $contrato->Pagado,2,',','.').' ('.$numero->traducir(($contrato->Valor_Inicial + $contrato->Valor_Adiciones) - $contrato->Pagado).')'), 'parrafo2', $alineacion_justificada);
	$seccion1->addTextBreak();
    ...
    foreach ($pagos as $pago):
		...
		$seccion1->addText((utf8_decode('FACTURA '.$pago->Numero_Factura.' ('.$contrato->Contratista.')')), 'parrafo2', $alineacion_izquierda);
    ...
    foreach ($pagos as $pago):
		...
		$seccion1->addText(utf8_decode('Acta Factura '.$pago->Numero_Factura.': Rete garantía (5% antes del IVA) $ '.number_format($pago->Valor_Retenido, 0, '', '.')), 'parrafo2', $alineacion_justificada);		
		
        $numero++;
    endforeach;

    $num = 1;
    foreach ($retenidos as $retenido):

			$seccion1->addText(utf8_decode('Valor Total Retenido:  $ '.number_format($retenido->Retenido, 0, '', '.')), 'parrafo3', $alineacion_justificada);		
	...
	$table = $seccion1->addTable();
	$table->addRow();
	$table->addCell(9000)->addText(utf8_decode('GERMAN IGNACIO VELEZ V.'), 'parrafo1', $alineacion_izquierda);
	$table->addCell(9000)->addText(utf8_decode(strtoupper($contrato->Representante_Legal)), 'parrafo1', $alineacion_izquierda);
	...
	$table->addRow();
	$table->addCell(9000)->addText(utf8_decode('DEVIMED S.A'), 'parrafo2', $alineacion_izquierda);
