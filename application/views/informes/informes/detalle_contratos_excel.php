...
//Encabezados
$objPHPExcel->getActiveSheet()->setCellValue('A1', "INFORME DETALLADO DE CONTRATOS");
...
$objPHPExcel->getActiveSheet()->setCellValue('K2', 'Plazo total');
$objPHPExcel->getActiveSheet()->setCellValue('L2', 'Valor adiciones');
$objPHPExcel->getActiveSheet()->setCellValue('M2', 'Valor pagado');
$objPHPExcel->getActiveSheet()->setCellValue('N2', 'Valor total');
$objPHPExcel->getActiveSheet()->setCellValue('O2', 'Saldo');
$objPHPExcel->getActiveSheet()->setCellValue('P2', 'Estado');
$objPHPExcel->getActiveSheet()->setCellValue('Q2', 'Centro de Costos');

/**
 * Estilos de los encabezados
 */
$objPHPExcel->getActiveSheet()->getStyle('A1:Q2')->applyFromArray($titulo_centrado_negrita);
$objPHPExcel->getActiveSheet()->getStyle('A2:Q2')->applyFromArray($bordes);
...
foreach ($contratos as $contrato) {
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('K'.$fila, $contrato->Plazo_Inicial +  $contrato->Plazo_Adiciones)        
        ->setCellValue('L'.$fila, $contrato->Valor_Adiciones)
        ->setCellValue('M'.$fila, $contrato->Pagado)
        ->setCellValue('N'.$fila, $contrato->Valor_Adiciones + $contrato->Valor_Inicial)
        ->setCellValue('O'.$fila, ($contrato->Valor_Adiciones + $contrato->Valor_Inicial) - $contrato->Pagado)
        ->setCellValue('P'.$fila, $contrato->Estado)
        ->setCellValue('Q'.$fila, $contrato->CentroCosto);
    ...
    $objPHPExcel->getActiveSheet()->getStyle("O{$fila}")->applyFromArray($estilos_numeros)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD);
 ...
 */
$objPHPExcel->getActiveSheet()->getStyle("A{$fila_inicial}:Q{$fila}")->applyFromArray($bordes);
